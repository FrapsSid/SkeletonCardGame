...
public class BettingRound {
    ...
    public void RegisterPlayer(Player player, Team team) {
        ...
        if (!playerStates.TryGetValue(player, out PlayerBetState state)) {
            state = new PlayerBetState(player);
    ...
    public void OpenBetting() {
        ...
        foreach (var state in playerStates.Values) {
            ...
            state.hasMatchedCurrentPrice = state.committedValue >= currentParticipationPrice;
        }
    ...
    public void MatchCurrentPrice(Player player, List<StakeAsset> assets) {
        ...
        state.hasMatchedCurrentPrice = true;
    ...
    public void RaiseCurrentPrice(Player player, int newPrice, List<StakeAsset> assets) {
        ...
        foreach (var activePlayer in activePlayers) {
            ...
            activeState.hasMatchedCurrentPrice = activePlayer == player || activeState.committedValue >= currentParticipationPrice;
        }
    ...
    public void UpgradeTarget(Player player, DeclaredCombinationTier newTarget, List<StakeAsset> optionalAssets) {
        ...
        if (!IsValidUpgrade(state.declaredTarget, newTarget)) {
    ...
    public void Pass(Player player) {
        ...
        if (state.committedValue < currentParticipationPrice) {
            throw new InvalidOperationException("Player can pass only after matching the current participation price");
        }
    ...
    public void Fold(Player player) {
        ...
        state.hasMatchedCurrentPrice = false;
    ...
    public bool AreAllActivePlayersMatched() {
        foreach (var player in activePlayers) {
            ...
            if (!state.hasMatchedCurrentPrice || state.committedValue < currentParticipationPrice) {
                return false;
            }
        }
    ...
    public void ResolvePot(List<Team> winners) {
        ...
        foreach (var state in playerStates.Values) {
            ...
            state.hasMatchedCurrentPrice = currentParticipationPrice == 0 && !state.hasFolded;
        }
    ...
    private void CommitAssets(Player player, PlayerBetState state, List<StakeAsset> assets) {
        ...
        foreach (var asset in assets) {
            ValidateStakeAsset(playerTeam, asset, seenThisAction);
            seenThisAction.Add(asset);
        }
    ...
    private void ValidateStakeAsset(Team playerTeam, StakeAsset asset, HashSet<StakeAsset> seenThisAction) {
        if (asset == null) {
            throw new ArgumentException("asset == null");
        }

        if (seenThisAction.Contains(asset)) {
            throw new InvalidOperationException("seenThisAction contains asset");
        }
        ...
        if (!asset.isAvailable || currentPot.Contains(asset)) {
    ...
    private static bool IsValidUpgrade(DeclaredCombinationTier currentTarget, DeclaredCombinationTier newTarget) {
        return (currentTarget == DeclaredCombinationTier.Easy && newTarget == DeclaredCombinationTier.Medium)
            || (currentTarget == DeclaredCombinationTier.Easy && newTarget == DeclaredCombinationTier.Hard)
            || (currentTarget == DeclaredCombinationTier.Medium && newTarget == DeclaredCombinationTier.Hard);
    }
